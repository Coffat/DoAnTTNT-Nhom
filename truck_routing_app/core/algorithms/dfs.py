...
class DFS(BaseSearch):
    ...
    def __init__(self, grid: np.ndarray):
        ...
        super().__init__(grid)
        self.stack = []
        self.visited_positions = set()  # Thay đổi: chỉ lưu vị trí đã thăm
    ...
    def search(self, start: Tuple[int, int], goal: Tuple[int, int]) -> List[Tuple[int, int]]:
        ...
        self.toll_stations_visited.clear()
        
        # Thêm vị trí bắt đầu vào stack
        self.stack.append(start)
        self.visited_positions.add(start)
        ...
        while self.stack:
            ...
            if current_pos == goal:
                ...
                if evaluation_result["is_feasible"]:
                    ...
                    # Đường đi hình học tìm được nhưng không khả thi (hết xăng)
                    # Vẫn trả về đường đi này và để phần UI hiển thị lý do không khả thi
                    self.current_path = path
                    self.path_length = len(path) - 1
                    self.current_fuel = 0  # Hết xăng
                    self.current_total_cost = evaluation_result["total_cost"]
                    self.current_fuel_cost = evaluation_result["fuel_cost"]
                    self.current_toll_cost = evaluation_result["toll_cost"]
                    return path
